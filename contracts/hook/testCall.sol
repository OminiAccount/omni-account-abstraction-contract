...
contract TestCall is BaseStruct {
    ...
    function callV3Swap1(
        V3SwapParams calldata params,
        address vizingSwapAddress
    ) external payable {
        ...
        (bool success, bytes memory data) = vizingSwapAddress.call{
            value: msg.value
        }(
    ...
    function callV3Swap2(
        ...
        (bool success, bytes memory data) = vizingSwapAddress.call{
            value: msg.value
        }(abi.encodeWithSelector(select, params));
        require(success, "v3Swap call failed");
        emit CallEvent(success, data);
    }

    function callV3Swap3(
        V3SwapParams calldata params,
        address vizingSwapAddress
    ) external payable {
        bool success;
        ...
        (success, data) = vizingSwapAddress.call{value: msg.value}(
            abi.encodeCall(IVizingSwap(vizingSwapAddress).v3Swap, params)
        );
        require(success, "v3Swap call failed");
        emit CallEvent(success, data);
    }
