...
contract ZKVizingAADataHelp is BaseStruct {
    ...
    function decodeCrossETHData(
        bytes memory callData
    ) external pure returns (CrossETHParams memory) {
        return abi.decode(callData, (CrossETHParams));
    }

    function decodeUniswapV2Data(
        bytes memory callData
    ) external pure returns (V2SwapParams memory) {
        return abi.decode(callData, (V2SwapParams));
    }

    function decodeUniswapV3Data(
        bytes memory callData
    ) external pure returns (V3SwapParams memory) {
        return abi.decode(callData, (V3SwapParams));
    }

    function decodePackedUserOperationGroup(
        bytes memory callData
    ) external pure returns (PackedUserOperation[] memory) {
        return abi.decode(callData, (PackedUserOperation[]));
    }

    function decodeCrossHookMessageParamsData(
        bytes memory callData
    ) external pure returns (CrossHookMessageParams memory) {
        return abi.decode(callData, (CrossHookMessageParams));
    }

    function decodeCrossMessageParamsData(
        bytes memory callData
    ) external pure returns (CrossMessageParams memory) {
        return abi.decode(callData, (CrossMessageParams));
    }

    function batchDecodeCrossHookMessageParams(
        bytes[] memory callDatas
    ) external pure returns (CrossHookMessageParams[] memory) {
        CrossHookMessageParams[] memory crossHookGroup;
        for (uint256 i; i < callDatas.length; i++) {
            crossHookGroup[i] = abi.decode(
                callDatas[i],
    ...
    function batchDecodeCrossMessageParams(
        bytes[] memory callDatas
    ) external pure returns (CrossMessageParams[] memory) {
        CrossMessageParams[] memory crossMessageGroup;
        for (uint256 i; i < callDatas.length; i++) {
            crossMessageGroup[i] = abi.decode(
                callDatas[i],
                (CrossMessageParams)
            );
        }
    ...
    function encodeCrossMessageParams(
        CrossMessageParams calldata params
    ) external pure returns (bytes memory crossMessage) {
        crossMessage = abi.encode(params);
    }

    function encodeCrossHookMessageParams(
        CrossHookMessageParams calldata params
    ) external pure returns (bytes memory bytesCrossHookMessageParams) {
        bytesCrossHookMessageParams = abi.encode(params);
    }

    function encodePackedUserOperationGroup(
        PackedUserOperation[] calldata paramsGroup
    ) external pure returns (bytes memory bytesUserOperation) {
        bytesUserOperation = abi.encode(paramsGroup);
    }

    function encodeCrossETHParams(
        CrossETHParams calldata params
    ) external pure returns (bytes memory bytesCrossETHAmount) {
    ...
    function getPackUint64s(
        uint64 high64,
        uint64 low64
    ) external pure returns (bytes16 packed) {
        packed = UserOperationLib.packUint64s(high64, low64);
    }

    function getPackOperation(
