...
contract ZKVizingAADataHelp is BaseStruct {
    ...
    function decodeCrossHookMessageParamsData(
        bytes memory callData
    ) external view returns (CrossHookMessageParams memory) {
        return abi.decode(callData, (CrossHookMessageParams));
    }

    function decodeCrossMessageParamsData(
        bytes memory callData
    ) external view returns (CrossMessageParams memory) {
    ...
    ) external view returns (bytes memory crossMessage) {
    ...
    ) external view returns (bytes memory bytesUserOperation) {
