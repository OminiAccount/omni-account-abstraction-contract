...
library GoldilocksPoseidon {
    ...
    // Require each input[i] is less than 2^256 - 2^64.
    function _hashNToMWithCap(
        uint256[] memory input,
        uint256[4] memory capacity,
        uint256 numOutputs
    ) internal pure returns (uint256[] memory output) {
        require(numOutputs < 8, "too many outputs");
        if (input.length == 0) {
        ...
        for (uint256 i = 0; i < 4; ) {
            state[i + 8] = capacity[i];
            ++i;
        }

        for (uint256 i = 0; i < numFullRound; ) {
            for (uint256 j = 0; j < SPONGE_RATE; ) {
                state[j] = input[i * SPONGE_RATE + j];
                ++j;
            }
            state = _permute(state);
    ...
    function hashNToMWithCap(
        ...
        output = _hashNToMWithCap(input, capacity, numOutputs);
    }
